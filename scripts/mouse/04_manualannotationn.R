...
combined <- readRDS(file.path(data_dir, "combined_processed_resolutions_REPRODUCED_umap.rds"))
markers <- readRDS(file.path(data_dir, "markers_res0.5.rds"))
...
write.csv(
  top_markers,
  file.path(data_dir, "top15_markers_res0.5_for_manual_annotation.csv"),
  row.names = FALSE
)
...
ggsave(
  filename = file.path(fig_dir, "manual_annotation_umap.png"),
  plot = p_manual,
  width = 10,
  height = 8,
  dpi = 300
...
ggsave(
  filename = file.path(fig_dir, "manual_annotation_umap_by_condition.png"),
  plot = p_manual_split,
  width = 14,
  height = 10,
  dpi = 300
