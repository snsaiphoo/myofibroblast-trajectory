...
combined <- readRDS(file.path(data_dir, "combined_singlets_counts_only.rds"))
...
saveRDS(
  combined,
  file.path(data_dir, "checkpoint_03_clusters.rds")
)
...
saveRDS(
  combined,
  file.path(data_dir, "combined_processed_resolutions_REPRODUCED.rds")
)
...
ggsave(
  file.path(fig_dir, "umap_resolutions.png"),
  combined_umap,
  width = 18,
  height = 12,
  dpi = 300
...
ggsave(
  file.path(fig_dir, "cluster_tree.png"),
  clust,
  width = 18,
  height = 12,
  dpi = 300
...
saveRDS(
  combined,
  file.path(data_dir, "combined_processed_resolutions_REPRODUCED_umap.rds")
)

ggsave(
  file.path(fig_dir, "umap_full.png"),
  umap_full,
  width = 8,
  height = 6,
  dpi = 300
...
ggsave(
  file.path(fig_dir, "umap_condition.png"),
  umap_condition,
  width = 8,
  height = 6,
  dpi = 300
...
saveRDS(
  combined,
  file.path(data_dir, "combined_processed_resolutions.rds")
)
...
write.csv(
  markers,
  file.path(fig_dir, "cluster_markers_res0.5.csv"),
  row.names = FALSE
)

write.csv(
  top_markers,
  file.path(fig_dir, "top10_markers_res0.5.csv"),
  row.names = FALSE
)
